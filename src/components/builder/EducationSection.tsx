  ...
  return (
    <div className="bg-surface rounded-2xl border border-surface-container p-5">
      <div className="flex justify-between items-center mb-3.5">
        <span className="text-[11px] font-bold tracking-widest uppercase" style={{ color: "#1d4ed8", fontFamily: "'Space Grotesk', system-ui, sans-serif" }}>
          활동 {index + 1}
        </span>
      ...
      <div className="grid gap-3" style={{ gridTemplateColumns: "1fr 1fr" }}>
        <div style={{ gridColumn: "1 / -1" }}>
          ...
          <TextInput placeholder="예) 삼성 청년 SW 아카데미 (SSAFY)" value={activity.title} onChange={(e) => onChange(activity.id, "title", e.target.value)} error={titleError} />
          {titleError && <p className="text-[12px] text-red-500 mt-1">필수 입력 항목입니다.</p>}
        </div>
        <div>
          <label style={labelStyle}>소속/기관</label>
          <TextInput placeholder="예) 삼성전자" value={activity.organization} onChange={(e) => onChange(activity.id, "organization", e.target.value)} />
        </div>
        <div>
          <label style={labelStyle}>활동 기간</label>
          <TextInput placeholder="예) 2023.03 ~ 2023.12" value={activity.period} onChange={(e) => onChange(activity.id, "period", e.target.value)} />
        </div>
        <div style={{ gridColumn: "1 / -1" }}>
